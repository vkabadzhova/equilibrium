...
impl Fluid {
    ...
    fn IX(x: u32, y: u32) -> usize {
        let x = &Fluid::constrain(x, 0, N - 1);
        let y = &Fluid::constrain(y, 0, N - 1);
        (x + (y * N)) as usize
    }
...
impl FluidSimulator {
    ...
    fn diffuse(&self, b: u32, x: &mut Vec<f32>, x0: &Vec<f32>, diffusion: &f32) {
        ...
        let a: f32 = &self.dt * diffusion * new_N_float * new_N_float;
    ...
    fn advect(
        ...
        let dtx: f32 = &self.dt * (N as f32 - 2f32);
        let dty: f32 = dtx;
        ...
        for j in 1..N - 1 {
            for i in 1..N - 1 {
                ...
                x = Fluid::constrain(x, 0.5, Nfloat - 1.0);
                y = Fluid::constrain(y, 0.5, Nfloat - 1.0);
    ...
    fn step(&self, fluid: &mut Fluid) {
        ...
        self.advect(1, &mut fluid.Vx, &fluid.Vx0, &fluid.Vx0, &fluid.Vy0);
        self.advect(2, &mut fluid.Vy, &fluid.Vy0, &fluid.Vx0, &fluid.Vy0);
        ...
        self.advect(0, &mut fluid.density, &fluid.s, &fluid.Vx, &fluid.Vy);
